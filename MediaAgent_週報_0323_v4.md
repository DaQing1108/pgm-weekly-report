...
| **P0** | **STT PRD Draft 完成** | Alex Liao | 2026/04/03 | 🟡 進行中（⑤） |
| **P0** | **FaceSearch PRD Draft 完成** | Alex Liao | 2026/04/03 | 🟡 進行中（⑤） |
| **P0** | **EDD 流程導入規劃 Draft 完成** | Alex Liao | 2026/04/03 | 🟡 進行中（⑤） |
...
| R6 | **Olapedia 黑箱問題（EDD 缺失）**：Olapedia/OlaMedia 系列 AI 模型清單、架構設計缺乏內部文件，有合規風險 | 🟡 中 | 進行中 | EDD 流程導入規劃 Draft 已啟動（Alex，4/3）；STT MRD ✅ + FaceSearch MRD ✅ 完成 | RD Lead 補齊 EDD + AI 模型清單 |
...
| R9 | **TVBS Release 路徑待確認**：NLE + AI + OpenCloud 整合需求評估中，Olapedia 1.0 已上線但 TVBS 正式 Release 節奏未定 | 🟡 中 | 進行中 | Steve 推進 Release 規劃；Demo 已完成 | Steve + Alex + TVBS 對齊節奏 |
| R10 | **Alex 文件產出集中（4/3 截止）**：STT PRD + FaceSearch PRD + EDD Draft 三份同時進行，單人負責，風險集中 | 🟡 中 | 新增（⑤） | PgM 追蹤器已登記，時程明確 | 評估是否請 Steve / Dream 協助撰寫 PRD 技術章節 |
| R11 | **5/9–5/10 活動規模不明**：創造栗/黎博提及但細節未確認，可能影響 Q2 資源配置 | 🟢 低 | 進行中 | 尚無具體 Action | 黎博 / 創造栗 4 月初確認細節 |
...
| 4 | STT MRD V5.0 評審（Team 內部）| Alex / Steve | 完成後安排 | ✅ MRD 完成（⑤，3/25–3/26） |
| **4a** | **Media_Agent_STT_PRD_Draft 完成** | **Alex Liao** | **2026/04/03** | **🟡 進行中（⑤）** |
| **4b** | **Media_Agent_FaceSearch_PRD_Draft 完成** | **Alex Liao** | **2026/04/03** | **🟡 進行中（⑤）** |
...
- *③ 260323_教育部門 Sales/PM/RD sync-up mtg（教育部門跨部門同步，2026-03-23）*
- *④ 260325_Agentic Meeting（Agentic Platform Discussion Meeting，2026-03-25）*
- *⑤ Program Manager Project Tracker.xlsx（PgM 個人追蹤器，Alex Liao，截至 2026-03-26）*
