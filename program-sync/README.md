...
| **跨瀏覽器同步** | 後端 `/api/state` 持久化全域狀態，不同瀏覽器看到相同資料 |
...
// 切換歷史週次（儀表板週次選擇器）
loadWeekView('W12');    // GET /api/weeks/W12 → 唯讀渲染
returnToCurrentWeek();  // 回到本週可編輯模式
```
...
5. `saveWeekState(curLabel, state)` 歸檔本週至 `/api/weeks/:weekLabel`
6. `renderWeekNav()` 渲染週次選擇器（從 `/api/weeks` 取得清單）
