...
| **週次歷史瀏覽** | Dashboard 可切換 W09–W13…，並透過 sessionStorage 同步到 Actions/Milestones/Input |
| **資料持久化** | 各週 JSON 存入 `backend/data/weeks/` 並納入 git，跨 Railway 部署永久保存 |
| **半自動週報歸檔** | Quick Input「週報歸檔」分頁：上傳 .md → 解析 → 預覽驗證 → 一鍵上傳並下載 JSON |
| **統一初始化模組** | `app-init.js` 管理 loading overlay、週次載入、sessionStorage 跨頁 context、歷史 banner |
...
└── assets/
    ...
    │   ├── app-init.js      # 統一初始化模組（actions/milestones/input 共用）
