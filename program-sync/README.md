...
```
儀表板 → 快速輸入 → Action Items → 里程碑 → 生成週報 → 審核流程
```
...
| 操作 | 觸發條件 | 效果 |
|------|---------|------|
| 送出審核 | Draft 狀態 | `reviewStatus = 'in-review'` |
| 核准 | In-Review 狀態 | `reviewStatus = 'approved'`，建立 Snapshot |
| 退回 | In-Review 狀態 | `reviewStatus = 'rejected'`，必須填寫備註 |
| 退回草稿 | Rejected 狀態 | 重置為 `draft`，可重新修改 |

**審核人**
- Alex Chen（Program Manager）
- Michael Wu（Product Director）
- Dream Lin（Develop Director）
...
| **v2.0** | 週報生成（AI + 本地）、審核流程、週快照、DOCX/PDF 匯出 |
