...
來源文件:  ① 260317_Program Progress Follow（Program Sync 週例會，2026-03-17，含本週 Follow-up）
           ...
           ⑤ Program Manager Project Tracker.xlsx（PgM ，Alex Liao，截至 2026-03-26）
```
...
**Top Wins：** Olapedia 1.0 已於本週正式上線，URL 為 `https://olamedia.olami.ai/`，累計入庫台北 3,500 小時 + 上海 6,000 小時以上廣播影片、732 位名人資料，支援人臉搜尋、STT 語言索引與多模態搜尋功能。**STT MRD V5.0 本週完成**（PgM 追蹤器確認，3/25–3/26），TVBS 節目部T觀點節目製作 WorkFlow 訪查同步完成，Swift Zhu 完成 SRT 字幕原型。PgM（⑤）確認三份文件製作進入進行中狀態：**STT PRD Draft**、**FaceSearch PRD Draft**、**EDD 流程導入規劃 Draft**，均由 Alex 負責，目標完成日 2026/04/03；另有 **TVBS AI Sharing Draft**（培訓類）進行中，目標 4/24。創造栗 AI 特攻隊兩岸均有技術進展：北京（Alger Wang + Bruce Zhong）完成情緒辨識 `emotion2VC` 同步；台灣（Sean Peng）啟動 Whisper V3 + LoRA 訓練流程，訓練截止日 3/27。AI Hackathon 確認 4/1 舉辦（13:30–16:30）。MBO Q2 目標今日（3/27）截止提交。

**Top Risks：** 小栗方 Pro IoT（智能體/IDE）整合仍在 Sean Peng、Tonny Shen、Michael Chien 三方協商中，尚需 Sean 發郵件確認，目前不確定性持續影響 Q2 排程。V2 Pro 市場現況落差大：目前僅有 8 套工程樣品，目標量為 2,000–3,000 套，ESP32 SDK 取得路徑需確認。客語 ASR 算力成本精算仍待定，NCHC Local AI Server 預計 4/9 就緒，Kingston/Raven 設備評估進行中。
...
**📋 PgM （⑤ 截至 2026/03/26，Excel）**

| 專案項目 | 項目內容 | 負責人 | 狀態 | 預計完成 |
|---------|---------|--------|------|---------|
| Training | TVBS_AI_sharing_Draft | Alex Liao | 🟡 進行中 | 2026/04/24 |
| Media Agent | 專案文件 EDD 流程導入規劃_Draft | Alex Liao | 🟡 進行中 | 2026/04/03 |
| Media Agent | Media_Agent_STT_PRD_Draft | Alex Liao | 🟡 進行中 | 2026/04/03 |
| Media Agent | Media_Agent_FaceSearch_PRD_Draft | Alex Liao | 🟡 進行中 | 2026/04/03 |
| MBO 績效管理 | VIA_26Q1~26Q2 MBO 季度評核啟動 | Alex Liao | 🟡 進行中 | 2026/03/27（今日）|
| Media Agent | Media_Agent_STT_MRD_V5.0 | Alex Liao | ✅ 已完成 | 3/25–3/26 |
| Media Agent | TVBS_節目部T觀點節目製作WorkFlow | Alex Liao | ✅ 已完成 | 3/25–3/26 |
| Media Agent | TVBS_作資源部（新聞組）現場流程訪查 | Alex Liao | ✅ 已完成 | 2026/03/24 |
| Media Agent | TVBS_節目部客戶訪談報告_260324 | Alex Liao | ✅ 已完成 | 2026/03/24 |
| Media Agent | Media_Agent_FaceSearch_MRD_V2.0 | Alex Liao | ✅ 已完成 | 2026/03/20 |
| Media Agent | OlaMedia_VM Server 申請 | Alex Liao | ✅ 已完成 | 2026/03/20 |
| MagicView | MagicView (MGV) 改版：BU2 RD 支援資源 | Alex Liao | ✅ 已完成 | 2026/03/20 |
...
Steve 在會議中展示平台 PPT，說明 OlaMedia（後端整合層）架構設計。Swift Zhu 完成 `SRT prototype.zip` 字幕原型，可於平台內直接展示字幕流程。
...
**狀態：** ✅ STT MRD V5.0 完成（3/25–3/26）；STT PRD Draft 啟動（目標 4/3）

本週 Alex 前往 TVBS（3/24），觀察節目部 STT 作業流程（TVBS_作資源部新聞組現場流程訪查 ✅ 完成），確認工具現況：TVBS 目前使用 Taption + Google Gemini + NotebookLM 等多工具組合，字幕成本高。TVBS 節目部客戶訪談報告_260324 ✅ 已完成，T觀點節目製作 WorkFlow ✅ 已完成。Dream 同步推進 OpenMAM 0.3 API 規格研究，作為後續 TVBS MAM 整合基礎。

**⬆️ PgM 更新（⑤）：**
- `Media_Agent_STT_MRD_V5.0` — ✅ **已完成**（3/25–3/26）
- `TVBS_節目部T觀點節目製作WorkFlow` — ✅ **已完成**（3/25–3/26）
- `Media_Agent_STT_PRD_Draft` — 🟡 **進行中**，Alex Liao，目標 **2026/04/03**
- `Media_Agent_FaceSearch_PRD_Draft` — 🟡 **進行中**，Alex Liao，目標 **2026/04/03**
...
**負責人/團隊：** Michael Chien（推動）/ RD Lead（執行）/ Alex（Draft 負責人）
**現況：** 共識達成，EDD 流程導入規劃 Draft 啟動（Alex，目標 4/3）
...
**⬆️ PgM 更新（⑤）：**
- `專案文件 EDD 流程導入規劃_Draft` — 🟡 **進行中**，Alex Liao，目標 **2026/04/03**
- `Media_Agent_FaceSearch_MRD_V2.0` — ✅ **已完成**（3/20，前置條件達成）

**文件進度鏈：** STT MRD ✅ + FaceSearch MRD ✅ → STT PRD 🟡（4/3）+ FaceSearch PRD 🟡（4/3）→ EDD 🟡（4/3）→ 開發
...
MBO Q2 全體提交截止今日（PgM 追蹤器：VIA_26Q1~26Q2 MBO 季度評核啟動 🟡 進行中，截止今日）。Steve Liu 已納入 Google Drive 提交流程（Agentic Meeting 確認），Alex 同步跟進。
...
**最重大里程碑：Olapedia 1.0 正式上線（olamedia.olami.ai）。** Steve 在 3/25 Agentic Meeting 完成平台 Demo，展示 PPT 說明 OlaMedia 架構，Swift 完成 SRT 字幕原型。**STT MRD V5.0 本週完成**（PgM 追蹤器確認，3/25–3/26），T觀點 WorkFlow 訪查 ✅，Alex 3/24 完成 TVBS 現場訪查（訪談報告 ✅）。Dream 推進 OpenMAM 0.3 規格。台標識別訓練四月初啟動，機房搬遷暫無急迫性（五月後）。EDD 工程文件規範共識達成，EDD Draft + STT PRD + FaceSearch PRD 三份文件均已啟動（Alex，截止 4/3）。TVBS AI Sharing 培訓 Deck 進行中（截止 4/24）。WER Benchmark 評測列入下週計劃。
...
MagicView 案件已關閉（PgM 追蹤器確認 3/20 完成，Pear 安排完成，Tom Liu 完成最終對接，VSTS branch 移交確認）。本週無其他重大更新。
...
Alex 完成 TVBS 現場訪查（3/24），確認節目部 STT 工具現況：
...
TVBS_作資源部（新聞組）現場流程訪查 ✅、TVBS_節目部客戶訪談報告_260324 ✅、T觀點節目製作 WorkFlow ✅ 均已完成，結果補入 STT MRD V5.0（已完成）。下週安排正式現場 STT 作業流程確認（下下週為目標）。
...
未來 RD 開工前必須完成 EDD，開發後補齊技術文件（含 AI 模型清單），從下一個開發 Round 起正式落實。目的：解決 Olapedia/OlaMedia「黑箱」問題。EDD 規劃 Draft 已啟動（Alex，4/3）。

**決策五：台標識別訓練四月初啟動，機房搬遷五月後進行（3/17 會議）**
台標訓練時程明確；機房搬遷無急迫性，與模型訓練時程連動，預估五月以後。

**決策六：MagicView 案件關閉（3/20 完成，3/25 確認）**
Pear 安排完成，VSTS branch 移交，此案正式 Close（PgM 追蹤器確認 3/20 完成）。
...
| **P0** | I Hackathon 執行** | Eddy / Ruru / 全體 | 2026/04/01 | 🎯 即將到來 |
| **P0** | STT PRD Draft | Alex Liao | 2026/04/03 | 🟡 進行中（⑤） |
| **P0** | FaceSearch PRD Draft | Alex Liao | 2026/04/03 | 🟡 進行中（⑤） |
| **P0** | EDD 流程導入規劃 Draft  | Alex Liao | 2026/04/03 | 🟡 進行中（⑤） |
| P0 | Sean 發郵件確認 IoT 伺服器連線 | Sean Peng | 本週 | ⏸️ 待執行 |
...
| P1 | TVBS AI Sharing 培訓 Draft 推進 | Alex Liao | 2026/04/24 | 🟡 進行中（⑤） |
...
|---------|------|---------|------|---------|--------|
| R1 | **小栗方 Pro IoT 整合阻塞**：Sean/Tonny/Michael 三方協商未完成，郵件尚未發出，影響 Q2/Q3 排程 | 🔴 高 | 進行中 | Sean 3/25 出席 Agentic Meeting，討論已啟動；郵件為最近阻塞點 | Sean 本週發郵件；Michael 確認 Q3 人力 |
| R2 | **客語 ASR 算力成本未定**：NCHC 4/9 才就緒，Local AI Server 採購評估中，若超 70 萬門檻則放棄 | 🔴 高 | 進行中 | Robert 負責設備採購（4/9）；TC + Eddy 精算中 | 黎博 本週決策（門檻 70 萬） |
| R3 | **V2 Pro 量產落差**：Engineering Sample 僅 8 套，目標 2,000–3,000 套，ESP32 SDK 未取得 | 🔴 高 | 進行中 | POC 評估 2–3 週進行中；SDK 待 Bruce 協調 | Kevin/Eva 決策量產節奏；Bruce 催 SDK |
| R4 | **創造栗 P2 升 P0/P1 資源不確定**：需 黎博 + Michael 正式拍板，目前懸而未決 | 🟡 中 | 進行中 | Sean 已進入 Program Sync，協商渠道建立；下週正式討論 | 黎博 + Michael 本週決策 |
| R5 | **Olapedia 黑箱問題（EDD 缺失）**：Olapedia/OlaMedia 系列 AI 模型清單、架構設計缺乏內部文件，有合規風險 | 🟡 中 | 進行中 | EDD 流程導入規劃 Draft 已啟動（Alex，4/3）；STT MRD ✅ + FaceSearch MRD ✅ 完成 | RD Lead 補齊 EDD + AI 模型清單 |
| R6 | **台灣 AI 特攻隊 Whisper 訓練樣本不足**：目前 ~450 筆 LoRA 樣本，8 類情緒分佈不均 | 🟡 中 | 進行中 | 持續蒐集；Gemini 2B + Gemini 3 Lite 備用方案 | Sean 今日完成訓練，評估準確率後決策 |
| R7 | **AI Hackathon 5 組 Demo 品質風險**：4/1 舉辦，時間緊迫（僅剩 4 天） | 🟡 中 | 進行中 | 各組 Demo 本週收斂，Ruru 統籌 | Eddy / Ruru 今日前確認各組就緒 |
| R8 | **TVBS Release 路徑待確認**：NLE + AI + OpenCloud 整合需求評估中，Olapedia 1.0 已上線但 TVBS 正式 Release 節奏未定 | 🟡 中 | 進行中 | Steve 推進 Release 規劃；Demo 已完成 | Steve + Alex + TVBS 對齊節奏 |
| R9 | **Alex 文件產出集中（4/3 截止）**：STT PRD + FaceSearch PRD + EDD Draft 三份同時進行，單人負責，風險集中 | 🟡 中 | 新增（⑤） | PgM 已登記，時程明確 | 評估是否請 Steve / Dream 協助撰寫 PRD 技術章節 |
| R10 | **5/9–5/10 活動規模不明**：創造栗/黎博提及但細節未確認，可能影響 Q2 資源配置 | 🟢 低 | 進行中 | 尚無具體 Action | 黎博 / 創造栗 4 月初確認細節 |
...
| 4 | STT MRD V5.0 審查（Team 內部）| Alex / Steve | 完成後安排 | ✅ MRD 完成（⑤，3/25–3/26） |
| **4a** | **Media_Agent_STT_PRD_Draft**  | **Alex Liao** | **2026/04/03** | **🟡 進行中（⑤）** |
| **4b** | **Media_Agent_FaceSearch_PRD＿Draft** | **Alex Liao** | **2026/04/03** | **🟡 進行中（⑤）** |
| 7 | 台標識別模型訓練啟動 | Steve 團隊 | 2026/04/初 | ⏸️ 待執行 |
| 8 | EDD 流程補充至 Olapedia / OlaMedia / STT 專案文件 | RD Lead + Alex | 下一 Round | 🟡 進行中（⑤ EDD Draft 啟動，4/3） |
| 9 | 台語 ASR 持續調研 | Anna | 長期 | 🟡 進行中 |
| **9a** | **TVBS_AI_sharing_Draft（培訓 Deck）完成** | **Alex Liao** | **2026/04/24** | **🟡 進行中（⑤）** |

#### 創造栗 — 軟體 & IoT（優先級：高）

| 項次 | 行動項目 | 負責人 | 目標時間 | PgM 狀態 |
|------|---------|--------|---------|---------|
| 10 | **Sean 發郵件 確認 IoT 伺服器連線** | Sean Peng | 本週 | ⏸️ 待執行 |
| 11 | IoT 整合 Q2排程，Michael 確認人力 | Michael Chien | 下週 | ⏸️ 待執行 |
...
| **2026/04/03** | **STT PRD Draft + FaceSearch PRD Draft + EDD 規劃 Draft 完成（Alex）** | Alex Liao | 🟡 進行中（⑤） |
...
| **2026/04/24** | **TVBS AI Sharing 培訓 Deck 完成（Alex）** | Alex Liao | 🟡 進行中（⑤） |
...
本週以 **Olapedia 1.0 正式上線（olamedia.olami.ai）** 為最重要里程碑——這是 VIA Media Agent 團隊從 PoC 到真實產品的關鍵跨越，732 名人庫 + 9,500 小時影片入庫，完整實現人臉搜尋 + STT + 多模態查詢三大功能。PgM 追蹤器（⑤）本次整合帶來兩項重要更新：其一，**STT MRD V5.0 已完成**（3/25–3/26），T觀點 WorkFlow 訪查 ✅，標誌 TVBS 需求調研階段告一段落；其二，STT PRD、FaceSearch PRD、EDD 規劃 Draft 三份文件同步啟動，截止 4/3，顯示 MRD → PRD → EDD 的文件鏈條正在系統性推進。今日（3/27）是本週的「執行收斂日」：MBO 提交、SEL Demo、Whisper 訓練三線同時截止，是 Q1 成果的最後收割節點。**下週的關鍵任務**明確集中在三件事：① 4/1 AI Hackathon 精彩呈現（最近的可見交付），② Sean 盡快完成 NCC 郵件申請（IoT 整合最小阻塞點），③ Alex 在 4/3 前完成三份 Draft 文件（PRD + EDD 文件鏈的核心前置，需評估是否分擔技術章節）。Olapedia 上線後，下一個清晰目標是 TVBS 正式 Release 路徑——NLE + AI + OpenCloud 的整合規劃需要本月確立。
...
- *③ 260323_教育部門 Sales/PM/RD sync-up mtg（教育部門跨部門同步，2026-03-23）*
- *④ 260325_Agentic Meeting（Agentic Platform Discussion Meeting，2026-03-25）*
- *⑤ Program Manager Project Tracker.xlsxPgM ，Alex Liao，截至 2026-03-26）*
