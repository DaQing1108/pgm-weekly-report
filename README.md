...
2. **開啟草稿確認內容無誤**（審閱專案狀態、Action Items、Risks、team 欄位）
3. **執行匯入指令** → 將草稿推送至 Railway

```bash
python3 scripts/import-draft.py backend/drafts/ProgramSync_W##_YYYY-MM-DD_draft.md
```

### AI 草稿生成（`program-sync-report` Skill）

在 Claude Code 說「生成 W## 週報草稿」即觸發。Skill 執行流程：

1. 讀取 `docs/組織架構與人員名單.md` 進行人名與 BU 校正
2. 逐一 `notion-fetch` 使用者提供的當週會議 URL
3. 解析使用者貼入的 PM Tasks（Open Tasks view）
4. 依 team 關鍵字自動分類（tv-solution / media-agent / chuangzaoli / learnmode）
5. 輸出 `backend/drafts/ProgramSync_{weekLabel}_{date}_draft.md`
